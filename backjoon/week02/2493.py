...
"""
n = int(s.readline())
towers = list(map(int, s.readline().split()))
stack = []

for i in range(n):
    currentHeight = towers[i]

    if stack:
        while stack:
            if stack[-1][0] > currentHeight:
                # 이전값이 현재 높이보다 크다?
                print(stack[-1][1] + 1, end = " ")
                break
            elif stack[-1][0] < currentHeight:
                # 이전값이 현재 높이보다 작다? 
                stack.pop()
                if not stack:
                    # 스택이 비었다면 신호를 송신해줄 탑이 없다.
                    print(0, end=" ")
                    break
            else:
                # 이전값이 현재값과 같다.
