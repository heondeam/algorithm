...
def find_answer():
    ...
    while start < end:
        # 두 포인터가 가리키는 값들의 합의 절댓값이 ans 보다 작으면 해당 값과 두 포인터를 기록함
        if abs(solutions[start] + solutions[end]) <= ans:
            ans = abs(solutions[start] + solutions[end])

            lt = start
            rt = end

        # 두 포인터가 가리키는 값들의 합이 음수면 왼쪽 포인터를 이동
        if solutions[start] + solutions[end] < 0:
            start += 1
        # 두 포인터가 가리키는 값들의 합이 양수면 오른쪽 포인터를 이동    
        elif solutions[start] + solutions[end] >= 0:
            end -= 1

    # 두 값의 합의 절댓값이 가장 작았던 두 수를 출력한다.
    print(solutions[lt], solutions[rt])
